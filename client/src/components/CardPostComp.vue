  ...
  <div class="card-post">
    <div class="main-post">
      ...
      <div class="embed">
        ...
        <p>{{ post.caption }}</p>
        <button
          v-if="userData !== {} && userData.id === post.UserId"
    ...
    <div class="like-comment">
      <a @click.prevent="likePostHandler(post.id)" href="" style="text-decoration: none">
        <img
      ...
      <a @click.prevent="toCommentPage(post.id)" href="">
        <img
...
export default {
  name: "CardPostComp",
  methods: {
    ...mapActions(["deletePost", "getPostDataById"]),
    async toCommentPage(id) {
      if(!this.isLoggedIn) {
        swal("please login first", "", "info");
      } else {
        await this.getPostDataById(id)
    ...
    likePostHandler(id) {
      if(!this.isLoggedIn) {
        ...
        console.log(id, 'likePostHandler');
      }
    },
